...
const Achievement = () => {
  ...
  return (
    <section id="achievement" className="container mx-auto px-4 pt-16 pb-8">
      <h2 className="text-3xl font-bold text-left mb-4 text-gray-900 dark:text-white">Achievements</h2>
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="text-gray-500 dark:text-white/60 mt-4">Loading achievements...</p>
        </div>
      ) : achievements.length === 0 ? (
        <div className="text-center py-20">
          <p className="text-gray-500 dark:text-white/60">No achievements available yet.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-8">
          {achievements.map((item) => (
            <div
              key={item.id}
              className="flex flex-col md:flex-row items-center bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-transparent p-4 hover:shadow-xl transition-transform duration-300 hover:scale-105"
            >
              {/* Gambar */}
              <div className="w-full h-64 overflow-hidden rounded-md flex-shrink-0 mb-4 md:mb-0 md:w-40 md:h-40 md:mr-6 bg-gray-50 dark:bg-[#1a1a1f]">
                <Image
